...
const PROFILE = {
  ...
  tagline:
    `Product-focused engineer with ${YEARS_EXP}+ years building scalable SaaS and healthcare platforms — end-to-end ownership from system design to production.`,
...
const HERO_BADGES = [`${YEARS_EXP}+ YEARS`, "JAVA · SPRING BOOT", "ANGULAR · REACT", "AWS · MICROSERVICES"];
...
const STATS: { to: number; prefix?: string; suffix?: string; decimals?: number; label: string }[] = [
  { to: YEARS_EXP, suffix: "+", decimals: 1, label: "Years of professional experience" },
...
const PROJECTS: Project[] = [
  {
    ...
    links: [
      ...
      { label: "Frontend", href: "https://github.com/ajkumarray/nirdeshak" },
    ],
  },
  {
  ...
  },
  {
    ...
    tags: ["Chrome Extension", "Manifest V3", "JavaScript", "GitHub API", "LeetCode"],
    links: [
  ...
  return (
    <section id="stats" className="section-band">
      <div className="mx-auto w-[min(92vw,1560px)]">
        <RevealStagger className="grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-[var(--site-border)] bg-[var(--site-border)] md:grid-cols-3 lg:grid-cols-5">
          {STATS.map((stat, i) => (
            <RevealItem key={stat.label} className={i === 0 ? "col-span-2 md:col-span-1" : undefined}>
              <motion.div
                whileHover={{ y: -5 }}
                transition={{ type: "spring", stiffness: 260, damping: 22 }}
                className="group relative overflow-hidden bg-[var(--site-card-strong)] p-6 text-center md:p-8 md:min-h-[210px]"
              >
      ...
      <div className="grid items-start gap-10 lg:grid-cols-[1.15fr_0.85fr]">
        <Reveal>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--site-muted)] md:text-xl">
            {`Full Stack Software Engineer with ${YEARS_EXP}+ years building scalable SaaS and healthcare platforms using Java,`}
          ...
          <div className="mx-auto mt-12 grid max-w-[85rem] grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
            {channels.map((channel, i) => (
              <a
                ...
                rel="noreferrer"
                onClick={channel.copy ? (e) => {
                  e.preventDefault();
                  try { navigator.clipboard.writeText(channel.value); } catch {}
                  push(`${channel.label} copied!`);
                } : undefined}
                className={clsx(
    ...
    <AnimatePresence>
      {visible && (
        <motion.button
          ...
          className="fixed bottom-6 right-6 z-[150] inline-flex h-11 w-11 items-center justify-center rounded-full bg-[var(--site-gold)] text-[var(--site-gold-ink)] shadow-[0_8px_28px_-6px_rgba(0,0,0,0.35)] transition hover:-translate-y-0.5 hover:bg-[var(--site-gold-bright)]"
        >
